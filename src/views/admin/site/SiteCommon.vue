    ...
    <div class="p-5">
        ...
        <div class="flex gap-3 justify-between items-center">
            ...
            <div class="flex gap-3">
                <preview-page />
                <create-page-dom
                    after-create-auto-hide
                    :firestoreDB="CommonDB"
                />
                <q-btn
                    label="儲存"
                    @click="handleSave()"
        ...
        <div>
            <template
                v-for="(dom, index) in pageDomsData"
                :key="dom.id+`${index}`"
            >
...
import { cloneDeep, find, get, has, isArray, isString, last, pick, set } from 'lodash';
...
// watch(()=>route.fullPath, ()=>router.go(0))
const Notify = useNotify()
const Dialog = useDialog()


const siteStore = useSiteStore()
const CommonDB = db().collection('Sites').doc(route.params.site_id as string).collection('Commons').doc(last(route.path.split('/')));
const CommonData = ref<PageType>(useFirestore(CommonDB) as any)
watchEffect(() => {
    console.log("CommonData.value", CommonData.value);

    if (CommonData.value) {
        siteStore.pageData = CommonData.value
    } else if (CommonData.value === null) {
        siteStore.pageData = {
            description: "",
            domsData: [],
            note: "",
            showName: route.meta.title as string ?? '',
            url: "",
        }
