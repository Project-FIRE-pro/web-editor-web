<template>
  ...
  <div class="container px-5 py-24 mx-auto">
    <div class="flex flex-wrap -m-4">
          <template v-for="item in (findByKey(props.data, [ 'BlogBlock']) as any) ">
          <div class="p-4 md:w-1/3">
        <div class="h-full border-2 border-gray-200 border-opacity-60 rounded-lg overflow-hidden">
          <img class="lg:h-48 md:h-36 w-full object-cover object-center" src="https://dummyimage.com/720x400" alt="blog">
          <div class="p-6">
            <h2 class="tracking-widest text-xs title-font font-medium text-gray-400 mb-1">{{ findByKey(item, [ 'SubTitle']) }}</h2>
            <h1 class="title-font text-lg font-medium text-gray-900 mb-3">{{ findByKey(item, [ 'Title']) }}</h1>
            <p class="leading-relaxed mb-3 whitespace-pre-line ">{{ findByKey(item, [ 'Note']) }}</p>
...
<script  lang="ts">
const componentId = 'rpOwzmzYR7Rq0Cm3fuKh'
